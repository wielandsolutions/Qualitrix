...
public class ReadTestDataFromExcel extends GetExcelFileData {
	...
	public static List <TestDataForContoso>  getTestDataForSunbird(String callingTestScript) 
	{
		List <TestDataForContoso> objListOfTestDataForSunbird = new ArrayList<TestDataForContoso>();
		int columnIndex = 0, sizeofHeaders = 0; 
		String columnName = "", dataValue = "";
		TestDataForContoso objTDForSunBird = new TestDataForContoso();
			...
			if (noOfRecordsToBeTested > 0 && headers.size() > 0 && data.size() > 0 ) {
				for (int row = 0; row < noOfRecordsToBeTested ; row++)	{
					...
					if (columnName != null)		
					{
						...
						else if(columnName.equals(location.trim()))
						objTDForSunBird.setLocation(dataValue);
						else if(columnName.equals(shiftType.trim()))
						objTDForSunBird.setShiftType(dataValue);
						else if(columnName.equals(day.trim()))
						objTDForSunBird.setDay(dataValue);
						else if(columnName.equals(profileOne.trim()))
						objTDForSunBird.setProfileOne(dataValue);
						else if(columnName.equals(profileTwo.trim()))
						objTDForSunBird.setProfileTwo(dataValue);
					}
					...
					if (columnIndex == sizeofHeaders)	{
						...
						columnIndex = 0;
						objTDForSunBird = new TestDataForContoso();					
					}
				} 
