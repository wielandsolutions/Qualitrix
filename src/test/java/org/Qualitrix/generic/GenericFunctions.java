...
import org.Qualitrix.startup.BaseTest;
...
public class GenericFunctions extends BaseTest{
    ...
    public static void waitForElementToDisAppear(WebElement blockingMessagePopup) {
        WebDriverWait wait = new WebDriverWait(driver, 30);
        wait.until(ExpectedConditions.invisibilityOfAllElements(blockingMessagePopup));
    }
    ...
    public WebElement waitForElementToDisAppear(WebElement id, int timeout) {
        WebDriverWait wait = new WebDriverWait(driver, timeout);
        wait.until(ExpectedConditions.visibilityOf(id));
        return id;
    }
